...
class GlobalStackableSpawner extends StackableSpawner {
	...
	protected function spawnEntity() : void {
		...
		for ($attempts = 0; $attempts < $this->spawnAttempts; $attempts++) {
			...
			if (!$world->isChunkLoaded($pos->x >> Chunk::COORD_BIT_SIZE, $pos->z >> Chunk::COORD_BIT_SIZE)) continue;
				...
				for ($x = $minX; $x <= $maxX; $x++) {
					for ($z = $minZ; $z <= $maxZ; $z++) {
						...
						foreach ($world->getChunkEntities($x, $z) as $target) {
							if (
								!$target instanceof StackableEntity ||
								!$target->isAlive() ||
								$target->isFlaggedForDespawn() ||
								$target instanceof GlobalStackableEntity &&
			...
			if ($entity instanceof StackableEntity) $entity->addStackSize($this->currentStack - 1);
			$entity->spawnToAll();
